...
- [x] Add `undue-notability` pattern to `backend/src/patterns/registry.ts`
...
- [x] Extend `backend/src/patterns/registry.test.ts`
...
- [x] Update existing meta tests
  - Adjust pattern count expectation (`PATTERNS.length` >= 46).
  - Update version assertion to the new `PATTERN_ENGINE_VERSION`.

## Documentation

- [x] Update `PATTERN_UPDATE_v1.1.0.md` (or successor) with a new v1.7.0 section summarizing the change.
- [x] Refresh `IMPLEMENTATION_SUMMARY.md` and any README references to pattern count/version.
- [x] Ensure new change is referenced in relevant OpenSpec specs/tasks if necessary.

## Validation

- [x] Run `npm test` within `backend/`.
- [x] Run `npm run lint` in `backend/`.
- [x] Run `openspec validate add-undue-notability-pattern --strict`.
